 ...
 * mode gates, then budget, then keyword rules (heavy wins over light), then
 * the short-task heuristic, then the peak-hour demotion. Everything else
...
function decideRoute({ text, config, sessionTokens, now = new Date() }) {
  ...
  const flash = routeOf(config.flashProvider, config.flashModel);
  if (flash !== undefined && hasKeyword(text, config.lightKeywords)) return { route: flash, trigger: "auto-light" };
