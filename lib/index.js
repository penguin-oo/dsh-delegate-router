// dsh-subagent-router — host half: deterministic Flash/Pro routing for
...
const WRAPPER_MARK = Symbol("dsh-subagent-router.wrapper");
...
export function apply(ctx, config = {}) {
  ...
  const effective = {
    ...
    flashProvider: config.flashProvider ?? env("DSH_SUBAGENT_ROUTER_FLASH_PROVIDER") ?? "",
    flashModel: config.flashModel ?? env("DSH_SUBAGENT_ROUTER_FLASH_MODEL") ?? "",
    proProvider: config.proProvider ?? env("DSH_SUBAGENT_ROUTER_PRO_PROVIDER") ?? "",
    proModel: config.proModel ?? env("DSH_SUBAGENT_ROUTER_PRO_MODEL") ?? "",
    ...
    const wrapper = {
      ...
      async execute(args, exec) {
        ...
        const decision = decideRoute({
          ...
          sessionTokens: 0, // budget cap activates once token accounting lands
        });
  ...
  const syncAll = () => {
    ...
    syncing = true;
    try {
      for (const agent of ctx.get("agents")?.list?.() ?? []) syncAgent(agent, true);
    } finally {
      syncing = false;
    }
  };
